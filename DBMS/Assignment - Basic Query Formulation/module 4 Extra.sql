...
select CustNo, CONCAT(CustFirstName," ",CustLastName) as Name, CustBal from Customer;
...
select CustNo, CONCAT(CustFirstName," ",CustLastName) as Name, CustCity, CustBal from Customer
where (CustCity = "Denver" AND CustBal>150)
or (CustCity = "Seattle" AND CustBal>300);

select OrdNo, OrdDate, OrderTbl.CustNo, CONCAT(CustFirstName," ",CustLastName) as Name from OrderTbl
inner join Customer on Customer.CustNo = OrderTbl.CustNo
where OrdDate between "2017-01-01" and "2017-01-31"
and CustState = "CO";
...
select CustCity,AVG(CustBal), count(*) as count from Customer
where CustState="WA"
group by CustCity
having count>1;
